...
PROMPT='%F{green}%n@%m%f [%F{blue}%~%f] CPU: $(ps -A -o %cpu | awk "{s+=$1} END {print s \"%\"}") MEM: $(vm_stat | awk "/Pages free/ {print \$3 * 4096 / 1024 / 1024 \" MB\"}") BAT: $(pmset -g batt | grep -Eo "\d+%") > '
...
alias klog="k logs -f"
alias utc="date -u"
alias x-redis='nix-shell -p redis --run "redis-server"'
alias x-redis-cli='nix-shell -p redis --run "redis-cli"'
alias x-psql="nix-shell -p postgresql"
alias dremio="/opt/dremio/bin/dremio"
...
alias flink-start="~/Downloads/flink-1.20.0/bin/start-cluster.sh"
alias flink-stop="~/Downloads/flink-1.20.0/bin/stop-cluster.sh"
alias flink-run="~/Downloads/flink-1.20.0/bin/flink run"
...
if [[ -n "$DEVBOX_SHELL" ]]; then
  ...
  export POWERLEVEL9K_CUSTOM_DEVBOX="echo '%F{blue}[devbox]λ%f '"
fi


typeset -g POWERLEVEL9K_INSTANT_PROMPT=quiet

export FLINK_CONF_DIR=~/.flink-config
export FLINK_CONF_DIR=~/.flink-config
